...
VOLUME_SUFFIXES=(
  ...
  "peer0.org2.fabric2.variant.com"
)
...
function clearContainers() {
  ...
  local network_filter="network=fabric_raft_variant_net"
  ${CONTAINER_CLI} rm -f $(${CONTAINER_CLI} ps -aq --filter label=raft-network=fabric2-channel-variant) 2>/dev/null || true
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    infoln "Creating Org2 Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.org2.fabric2.variant.com org2
    peer_cert admin Admin@org2.variant.com org2
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/org1.variant.com/
    COUNTER=0
    rc=1
    while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
      sleep 1
      set -x
      fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7154 --caname ca-org1 --tls.certfiles "${PWD}/organizations/fabric-ca/org1/ca-cert.pem"
      res=$?
    { set +x; } 2>/dev/null
    rc=$res  # Update rc
    COUNTER=$((COUNTER + 1))
