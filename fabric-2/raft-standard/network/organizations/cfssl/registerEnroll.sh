...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.standard.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.standard.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.standard.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.standard.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.standard.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.standard.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.standard.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.standard.com-cert.pem"
...
function orderer_cert() {
    ...
    mkdir -p organizations/ordererOrganizations/standard.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/standard.com/orderers
    mkdir -p organizations/ordererOrganizations/standard.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/standard.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.standard.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.standard.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/standard.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/standard.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.standard.com-cert.pem"
