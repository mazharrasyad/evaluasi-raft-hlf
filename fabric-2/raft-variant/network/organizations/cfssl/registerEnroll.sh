...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.variant.com/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.variant.com/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.variant.com/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.variant.com/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.variant.com/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.variant.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.variant.com-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.variant.com-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.fabric2.variant.com

    mkdir -p organizations/ordererOrganizations/variant.com/ca
    mkdir -p organizations/ordererOrganizations/variant.com/msp/cacerts
    mkdir -p organizations/ordererOrganizations/variant.com/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/variant.com/orderers
    mkdir -p organizations/ordererOrganizations/variant.com/tlsca

    CERT_DIR=organizations/ordererOrganizations/variant.com

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.variant.com-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.variant.com-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/variant.com/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/variant.com/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.variant.com-cert.pem"
