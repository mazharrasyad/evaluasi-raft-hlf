...
function printHelp () {
  ...
  echo "    -c <channel name> - test network channel name (defaults to \"channel-standard\")"
  ...
  echo "	addOrg3.sh up"
  echo "	addOrg3.sh up -c channel-standard -s couchdb"
...
function generateOrg3Definition() {
  ...
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.fabric3.standard/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.fabric3.standard/peers/peer0.org3.fabric3.standard/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.fabric3.standard" ]; then
    generateOrg3
...
#default for delay
CLI_DELAY=3
# channel name defaults to "channel-standard"
