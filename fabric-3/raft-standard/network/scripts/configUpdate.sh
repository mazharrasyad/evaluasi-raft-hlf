...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${TEST_NETWORK_HOME}/channel-artifacts/config_block.pb -o localhost:7050 --ordererTLSHostnameOverride orderer.fabric3.standard -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
