...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"channel-variant\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c channel-variant -s couchdb "
    println "   network.sh up createChannel -bft -c channel-variant -s couchdb "
