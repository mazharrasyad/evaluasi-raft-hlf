...
In your 'monitoring' window, run this to watch all activity from the all the docker containers on the `raft_net` network; this will monitor all the docker containers that are added to the `raft_net` network. The network is usually created by the `./network.sh up` command, so remember to delay running this until at least the network is created. It is possible to precreate the network with `docker network create raft_net` if you wish.
...
This is very similar to the `deployCC` command, it needs the name, and path. But also needs to have the port the chaincode container is going to use. As each container is on the `raft_net` network, you might wish to alter this so there are no collisions with other chaincode containers.
...
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/org1.raft/tlsca/tlsca.org1.raft-cert.pem
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/org1.raft/users/Admin@org1.raft/msp
export CORE_PEER_ADDRESS=localhost:7153
export PATH=${PWD}/../bin:$PATH
export FABRIC_CFG_PATH=${PWD}/../config

# invoke the function
peer chaincode query -C raft -n basicts -c '{"Args":["org.hyperledger.fabric:GetMetadata"]}' | jq
```
...
```bash
    docker run --rm -d --name fabric3standard_peer0org1_assettx_ccaas  \
                  --network raft_net \
...
Not starting docker containers; these are the commands we would have run
    docker run --rm -d --name fabric3standard_peer0org1_basicj_ccaas                    --network raft_net                   -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999                   -e CHAINCODE_ID=basicj_1.0:59dcd73a14e2db8eab7f7683343ce27ac242b93b4e8075605a460d63a0438405 -e CORE_CHAINCODE_ID_NAME=basicj_1.0:59dcd73a14e2db8eab7f7683343ce27ac242b93b4e8075605a460d63a0438405                     basicj_ccaas_image:latest
```
...
```bash
 docker run --rm -it -p 9229:9229 --name fabric3standard_peer0org2_basic_ccaas --network raft_net -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 -e CORE_CHAINCODE_ID_NAME=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 basic_ccaas_image:latest
```

Java for example, could be started like this

```bash
 docker run --rm -it --name fabric3standard_peer0org1_basicj_ccaas -p 8000:8000 --network raft_net -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basicj_1.0:b014a03d8eb1898535e25b4dfeeb3f8244c9f07d91a06aec03e2d19174c45e4f -e CORE_CHAINCODE_ID_NAME=basicj_1.0:b014a03d8e
...
- to avoid collisions with other RAFT environments the helper scripts prefix container names with `fabric3standard_`
...
```json
{
  "address": "fabric3standard_{{.peername}}_assettransfer_ccaas:9999",
  "dial_timeout": "10s",
  "tls_required": false
}
```
...
The external builder will then resolve this address to be `fabric3standard_org1peer1_assettransfer_ccaas:9999` for the peer to use.
