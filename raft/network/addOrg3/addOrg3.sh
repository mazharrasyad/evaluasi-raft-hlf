...
function printHelp () {
  ...
  echo "    -c <channel name> - test network channel name (defaults to \"raft\")"
  ...
  echo "	addOrg3.sh up"
  echo "	addOrg3.sh up -c raft -s couchdb"
...
function generateOrg3Definition() {
  ...
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.raft/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.raft/peers/peer0.org3.raft/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.raft" ]; then
    generateOrg3
