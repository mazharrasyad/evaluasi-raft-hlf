...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"raft\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c raft -s couchdb "
    println "   network.sh up createChannel -bft -c raft -s couchdb "
