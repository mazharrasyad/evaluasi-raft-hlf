...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"fabric3-channel-variant\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c fabric3-channel-variant -s couchdb "
    println "   network.sh up createChannel -bft -c fabric3-channel-variant -s couchdb "
