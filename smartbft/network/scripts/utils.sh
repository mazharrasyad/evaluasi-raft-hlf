...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"smartbft\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c smartbft -s couchdb "
    println "   network.sh up createChannel -bft -c smartbft -s couchdb "
