...
componentLoaderReady.then(() => {
    ...
    const blocksTableContainerEl = document.getElementById('blocksTableContainer');
    ...
    function createBlocksDataTable(networksData) {
        ...
        if (!networksData || networksData.length === 0) {
            const emptyState = document.createElement('div');
            emptyState.className = 'flex flex-col items-center gap-3 rounded-2xl border border-white/10 bg-surfaceMuted/70 p-12 text-center';
            emptyState.innerHTML = `
                <svg class="h-12 w-12 text-textdark/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path>
                </svg>
                <p class="text-textdark/60">Tidak ada data blok yang tersedia</p>
            `;
        ...
        networksData.forEach((network, networkIndex) => {
            ...
            const networkBadge = document.createElement('div');
            networkBadge.className = 'inline-flex items-center gap-2 rounded-lg border px-4 py-2';
            const colors = networkColors[network.targetId] || { bg: 'bg-gray-500/15', text: 'text-gray-400', border: 'border-gray-500/30' };
            networkBadge.className += ` ${colors.bg} ${colors.text} ${colors.border}`;
            networkBadge.innerHTML = `
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"></path>
                </svg>
                <span class="font-semibold">${network.label || 'Unknown Network'}</span>
            `;

            networkTitle.appendChild(networkBadge);

            const blockCount = document.createElement('div');
            blockCount.className = 'text-sm text-textdark/60';
            blockCount.innerHTML = `
                <span class="font-mono font-semibold text-primary">${numberFormatter.format(network.blockHeight || 0)}</span>
    ...
    async function loadBlocksData() {
        try {
            // Show loading state
            if (blocksTableContainerEl) {
                blocksTableContainerEl.innerHTML = `
                    <div class="flex items-center justify-center bg-surfaceMuted/50 p-12">
                        <div class="text-center">
                            <svg class="mx-auto mb-3 h-8 w-8 animate-spin text-primary" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            ...
                            <p class="text-sm text-textdark/70">Memuat data blok dari semua network...</p>
                        </div>
                    </div>
                `;
            }
            ...
            // Render blocks table
            if (blocksTableContainerEl && data.results && Array.isArray(data.results)) {
                blocksTableContainerEl.innerHTML = '';
                const blocksTable = createBlocksDataTable(data.results);
                blocksTableContainerEl.appendChild(blocksTable);
            }
    ...
    if (refreshBlocksBtn) {
        refreshBlocksBtn.addEventListener('click', async () => {
            ...
            refreshBlocksBtn.querySelector('svg').classList.add('animate-spin');

            await loadBlocksData();

            refreshBlocksBtn.disabled = false;
            refreshBlocksBtn.querySelector('svg').classList.remove('animate-spin');
        });
    }
