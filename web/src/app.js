...
app.post('/api/shutdown-network', async (req, res) => {
    ...
    // Always clear ALL data when any network is shutdown to ensure data consistency
    // and avoid showing stale simulation data when networks are not running
    if (successCount > 0) {
        try {
            await clearSimulationData(null);
            console.log('[Shutdown] Cleared all simulation data after network shutdown');
        } catch (clearError) {
