...
// ==================== API PELAPORAN (Referensi SHM) ====================

// Helper untuk mendapatkan networkId dari request
function getNetworkIdFromRequest(req) {
    ...
    return networkMap[networkParam] || networkParam || 'channel-standard';
}

// Health check untuk pelaporan API
app.get('/api/pelaporan/health', async (req, res) => {
    try {
        const networks = ['channel-standard', 'channel-variant', 'channel-fabric3-standard', 'channel-fabric3-variant'];
        const healthChecks = [];
        ...
        res.json({
            error: false,
            message: allHealthy ? 'All networks are healthy' : 'Some networks are unhealthy',
            data: {
...
// Blockchain info untuk pelaporan
app.get('/api/pelaporan/blockchain/info', async (req, res) => {
    try {
        ...
        const currentBlockHash = crypto.createHash('sha256').update(dataString + Date.now()).digest('hex').substring(0, 32);
        const previousBlockHash = crypto.createHash('sha256').update(dataString).digest('hex').substring(0, 32);
        const genesisHash = crypto.createHash('sha256').update('genesis-block').digest('hex').substring(0, 32);
    ...
    try {
        ...
        if (result.success) {
            ...
            if (record) {
                ...
                const recordHash = crypto.createHash('sha256').update(dataString).digest('hex');
            ...
            if (recordTransactions.length > 0) {
                ...
                const history = recordTransactions.map(tx => ({
                    txId: tx.blockchainMetadata?.transactionId?.substring(0, 16) ||
                          crypto.createHash('sha256').update(`tx-${recordId}-${tx.timestamp}`).digest('hex').substring(0, 16),
                    ...
                    value: tx,
                    hash: crypto.createHash('sha256').update(JSON.stringify(tx)).digest('hex').substring(0, 16)
...
app.get('/api/pelaporan/blockchain/verify-chain', async (req, res) => {
    try {
        const networkId = getNetworkIdFromRequest(req);
        const result = await queryAllTransactionsFromBlocks(networkId);
...
app.post('/api/pelaporan/blockchain/fix-chain', async (req, res) => {
    try {
        const networkId = getNetworkIdFromRequest(req);
        const result = await queryAllTransactionsFromBlocks(networkId);
